...
MVP uses pre-registration: an admin creates the study or cohort entity in Usher (owner, custodians, category config, field-value identifier) before any data is submitted. Submission of unregistered cohorts is rejected. See `.dev/design/to-discuss.md` (Ownership and custodianship section) for the full description.
